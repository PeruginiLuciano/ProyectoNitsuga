...
class _CircleStateButtonState extends State<CircleStateButton> {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onPanUpdate: (details) {
        if (widget.enabled && !onTapUpCalled && (details.localPosition.dx < 0 || details.localPosition.dx > (widget.size ?? 70) || details.localPosition.dy < 0 || details.localPosition.dy > (widget.size ?? 70))) {
          onTapUpCalled = true;
          widget.onTapUp?.call();
        }
      },
